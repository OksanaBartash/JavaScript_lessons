...
function getAllSubsets(arr: number[]): number[][] {
	 ...
	 */
	function makeSubset(index: number, currentSubset: number[]): void {
		if (index === arr.length) {
...
const currentNumbers: number[] = [1, 2, 3]
 ...
 */
function getIteratorMonthNumbers(
	monthCounter: number,
	min: number,
	max: number
): () => number {
	return function getNextMonth(): number {
		if (monthCounter === max) {
			monthCounter = min
		} else {
			monthCounter++
...
const startIteratorMonth: () => number = getIteratorMonthNumbers(0, min, max)
 ...
 */
function showMonthsRepeat(numberRepeat: number): void {
 ...
 */
function getRandomNamber(): number {
...
function additionSum(): void {
  ...
  let timer = setInterval(() => {
    ...
    const number_1: number = getRandomNamber()
    const number_2: number = getRandomNamber()
    const correctSum: number = number_1 + number_2

    const userInputSum: string | null = prompt(
      `Введи суму двох чисел ${number_1} і ${number_2}`
    )
    const userSum: number = userInputSum !== null ? parseFloat(userInputSum) : NaN

    if (correctSum === userSum)
      console.log(
        `Вітаю! Ви дали вірну відповідь — сума чисел ${number_1} та ${number_2} складає ${correctSum}`
      )
    else
      console.log(
        `Ви помилились, сума чисел ${number_1} та ${number_2} складає ${correctSum}`
      )
