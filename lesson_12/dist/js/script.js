...
const arrNumbers = [2, 11, 45, 2, 67, 3, 23, 90];
 ...
 */
function createRundomArray(elemNumber) {
 ...
 */
function createRundomArray3(elemNumber) {
    return Array.from({ length: elemNumber }, () => Math.floor(Math.random() * 100));
}
...
const arrNumbers4 = [2, 11, 45, 2, 67, 3, 23, 90];
...
do {
    changeArray4 = false;
    for (let i = 1; i < arrNumbers4.length; i++) {
 ...
 */
function createRundomArray4(elemNumber) {
    return Array.from({ length: elemNumber }, () => Math.floor(Math.random() * 100));
}
 ...
 */
function createRundomArr4(elemNumber) {
    return Array.from({ length: elemNumber }, () => Math.floor(Math.random() * 100));
}
...
const namesArray = ['Toly', 'Rita', 'Olga', 'Andriy', 'Roman', 'Stepan', 'Olena', 'Ivan'];
...
if (index !== -1)
    ...
    console.log(`No, ${nameOlga} no fond`);
...
const arrNames = ['Toly', 'Rita', 'Olga', 'Andriy', 'Roman', 'Stepan', 'Olena', 'Ivan'];
console.log(namesArray);
/**
 ...
 * @returns
 */
function findLenghName(array, nameLength) {
    let left = 0;
    let right = array.length - 1;
    while (left <= right) {
        const middle = Math.floor((left + right) / 2);
        if (array[middle].length === nameLength)
            return middle;
        if (array[middle].length < nameLength)
...
if (name5simbol === -1)
    ...
    console.log(`Ім’я з 5-ти символів ${arrNames[name5simbol]} у масиві під індексом ${name5simbol}`);
console.log(` `);
//================================================================
//================================================================
...
function createMatrix(row, col) {
    return Array.from({ length: row }, () => Array.from({ length: col }, () => (Math.floor(Math.random() * 7) + 1)));
}
const weekMatrix = createMatrix(4, 8);
console.log(weekMatrix);
const numberSundayPerRow = weekMatrix.map(row => row.reduce((count, day) => (day === 7 ? count + 1 : count), 0));
